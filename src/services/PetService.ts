...
export class PetService {
  ...
  async delete(
    ...
    await this.petPhotoService.deleteAll(deletedPet, keepPhotos);
    await Pet.delete(id);
    return deletedPet;
  }
  ...
  async updateCommonValuesTable(): Promise<void> {
    ...
    for (let object of allBreeds) {
      ...
      const maxGenderObject = genderResult.reduce(
        (prev, current) =>
      ...
      const sizeResult = await Pet.createQueryBuilder("pet")
        ...
        .groupBy("pet.size")
        .getRawMany();
      const maxSizeObject = sizeResult.reduce(
        (prev, current) =>
          parseInt(prev.count) > parseInt(current.count) ? prev : current,
        0
      );
      ...
      const colorResult = await Pet.createQueryBuilder("pet")
        ...
        .groupBy("pet.color")
        .getRawMany();
      const maxColorObject = colorResult.reduce(
        (prev, current) =>
          parseInt(prev.count) > parseInt(current.count) ? prev : current,
        0
      );
